...
const char* GetMD5String(const char *msg, int mlen) {
    static char str[33];  // Static to persist after function return
    ...
    for (int j = 0; j < 4; j++) {
        u.w = d[j];
        char s[9];  // Temporary buffer for each chunk of MD5 hash
        snprintf(s, sizeof(s), "%02x%02x%02x%02x", u.b[0], u.b[1], u.b[2], u.b[3]);
        strcat(str, s);
    }
